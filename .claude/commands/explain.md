...
4. 段落ごとに英文・日本語訳・文構造（structures）・注釈（annotations）を生成
5. JSON を書き戻す
  ...
  "paragraphs": [
    {
      ...
      "structures": [
        {
          "text": "文節テキスト（英文中の該当部分）",
          "role": "S / V / O / C / M / 従属節",
          "label": "日本語ラベル（主語 / 動詞 / 目的語 / 補語 / 修飾句 / 従属節 等）"
        }
      ],
      "annotations": [
        {
...
文構造（structures）の role:
- `S`: 主語（青）
- `V`: 動詞（赤）
- `O`: 目的語（緑）
- `C`: 補語（紫）
- `M`: 修飾句（オレンジ）- 前置詞句、副詞句、例示句など
- `従属節`: 従属節（グレー）- if, when, while, because などで始まる節
